...
const Profile = ({customClaims}) => {
  ...
  const handleUpload = () => { // Image upload to firebase
    const promises = [];
    images.map((image) => {
      const storageRef = ref(storage, `agencies/logo_${agencyId}_${new Date().getTime().toString()}.png`)
  ...
  // NAME CHANGE
	const handleNameChange = (e) => { // Handler: new agency NAME
    e.preventDefault()
  ...
  const handleStateChange = (e) => { // location STATE
      ...
      {customClaims.agency && // agency settings
        <div className='z-0 flex-col p-16 pt-10 bg-slate-100'>
          ...
          <div className='w-full h-auto'>
                  ...
                  <div className={`${ style.inputSelect } bg-white col-span-3 flex items-center`}>
                      <label>
                        ...
                        <input className={`${style.fileUploadButton}`}
                          ...
                          ref={imgPicker}
                      />
                      {errors.images && images === null && (<span className="text-red-500">{errors.images}</span>)}
                      </label>
                      <div className='col-span-2'>
                        {imageURLs.map((url,i = self.crypto.randomUUID()) => (
                          <Image src={url} key={i} width={100} height={50} className='inline' alt={`image-upload-${ i }`} />
                        ))}
                      </div>
            ...
            <button onClick={handleSubmitClick} className={style.button} type="submit">Update Agency</button>
          </form>
          </div>
        </div>
      }
