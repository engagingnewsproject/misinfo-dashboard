 ...
 * - classification/swing_llm/prompts/meatiness.md
 */
...
/** @typedef {'electionSystemPrompt' | 'swingMultiPrompt' | 'swingSinglePrompt' | 'meatinessPrompt'} PipelinePromptKey */
 ...
 * @property {string} meatinessPrompt
 */
...
export const PIPELINE_PROMPT_FIELDS = [
	...
	{
		key: 'meatinessPrompt',
		label: 'Meatiness prompt',
		description:
			'Step 5 LLM score for curation ranking (how useful the article is for a state newsroom). Must keep {state}.',
		requiredPlaceholders: ['state'],
	},
]
...
export const PROMPT_DEFAULTS = {
	...
	meatinessPrompt: "Evaluate the article for the relevant state: {state}.\n\nHow much substantive information would a newsroom learn about election content clearly tied to that state, while also considering how much of the article is devoted to that content rather than unrelated material?\n\nMeatiness therefore reflects both:\n- The absolute amount of substantive state-election information in the article\n- The concentration of that information within the article.\n\nWhen comparing articles, ask both: How much useful state-specific election information would be lost if this article were excluded? and How much unrelated material would a newsroom need to work through to obtain that information?\n\nDo not evaluate either dimension in isolation.\n\nAmount of substantive state-election content\nAssess how much substantive information the article provides about the 2026 election within the state. Substantive information is information that adds to a newsroom's understanding of the election rather than merely mentioning an election, candidate, or official.\n\nSubstantive information can include the following related to the relevant state:\n- polling data or voting results\n- campaign developments\n- voting process coverage (deadlines, ballot access rules, etc.)\n- a named local candidate, official, or election administrator\n\nConcentration of state-election content\nAssess how much of the article is meaningfully devoted to 2026 election content related to the relevant state.\n\nContent should contribute little or nothing to this dimension when it consists primarily of:\n- political developments not related to the 2026 election\n- discussion of elections in other states\n- boilerplate content (e.g., ads, author bio)\n\nDo not require every sentence to concern the election. Relevant explanatory context may contribute to meatiness when it helps a reader understand the state-election story (e.g., issue stances and background of a candidate running in the 2026 election).\n\nMeatiness rubric\nWithin each rubric range, assign higher scores as the amount and concentration of substantive state-election information increase.\n\n0.75–1.00: High meatiness. Provides a substantial amount of substantive information about the 2026 election clearly tied to the relevant state, with much of the article meaningfully devoted to that content. Excluding the article would result in the loss of considerable useful state-election information, while a newsroom would need to work through relatively little unrelated material to obtain it.\n\n0.50–0.74: Moderate meatiness. Provides meaningful substantive information about the 2026 election clearly tied to the relevant state, but the amount of useful information is more limited, the relevant content constitutes only part of the article, or both. Excluding the article would result in the loss of useful information, although a newsroom may need to work through a meaningful amount of unrelated material to obtain it.\n\n0.25–0.49: Low meatiness. Provides some substantive information about the 2026 election clearly tied to the relevant state, but relatively little useful information would be lost if the article were excluded. The relevant coverage may be thin, brief, repetitive, substantially diluted by unrelated material, or some combination of these characteristics.\n\n0.00–0.24: Very low meatiness. Provides little substantive information about the 2026 election clearly tied to the relevant state, has very little of the article meaningfully devoted to that content, or both. Excluding the article would result in little loss of useful state-election information for a newsroom.\n\nOutput\nReturn JSON with only:\n- meatiness (number from 0 to 1)\n"
}
...
export function normalizePipelinePrompts(raw) {
	...
	return {
		...
		meatinessPrompt: normalizePromptText(source.meatinessPrompt),
	}
}
